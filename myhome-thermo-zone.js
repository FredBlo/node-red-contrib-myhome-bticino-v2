    ...
    this.processReceivedBUSFrames = function (msg, frame) {
      ...
      if (!config.skipevents || forceRefreshAndMsg) {
        ...
        if (!config.smartfilter || newPayloadinfo !== node.lastPayloadInfo || forceRefreshAndMsg) {
          ...
          let registeredPoint = mhcommon.findRegisteredPoint (gateway.points, 'thermozone', 'private_riser', config.zoneid, false);
          msg.mh_nodeConfigInfo = {
            ...
            'pointid_UI' : mhcommon.buildAPL_Dotted (config.zoneid, true) , // true = raw ID, no A.PL split
            'gateway' : {
              'name' : gateway.name ,
              'host' : gateway.host ,
              'port' : gateway.port
